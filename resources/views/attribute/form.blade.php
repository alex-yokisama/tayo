{{-- <form class="" action="" method="post">
    @csrf
    <input type="hidden" name="id" value="{{ isset($attribute) ? $attribute->id : ''}}"><br>
    <input type="hidden" name="backUrl" value="{{ $backUrl }}"><br>
    <input placeholder="name" type="text" name="name" value="{{ isset($attribute) ? $attribute->name : old('name') }}"><br>
    <select class="" name="type">
        @foreach ($types as $type_id => $type_name)
            <option
                value="{{ $type_id }}"
                {{ isset($attribute) && $attribute->type == $type_id ? 'selected' : ''}}
            >
                {{ $type_name }}
            </option>
        @endforeach
    </select><br>
    <select class="" name="measure">
        <option value="0">-- select --</option>
        @foreach ($measures as $measure)
            <option
                value="{{ $measure->id }}"
                {{ isset($attribute) && isset($attribute->measure) && $attribute->measure->id == $measure->id ? 'selected' : ''}}
            >
    ...
    @for ($i = 0; $i < 10; $i++)
        <div class="">
            @if (isset($attribute) && isset($attribute->options[$i]))
                <input placeholder="option" type="text" name="options[id_{{ $attribute->options[$i]->id }}]" value="{{ $attribute->options[$i]->name }}">
            @else
                <input placeholder="option" type="text" name="options[]" value="">
            @endif
        </div>
    @endfor
    ...
    <form class="editItemForm" action="" method="post" x-data="{}" @submit-save-form.window="document.querySelector('form.editItemForm').submit()">
        ...
        <x-form.container x-data="attributeForm()" x-init="typeChange">
            ...
            <x-form.input x-show="showOptions">
                ...
                <x-common.button.a href="#" @click="addOption">More</x-common.button.a>
            </x-form.input>
        </x-form.container>
        <script>
